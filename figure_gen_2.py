...
def process(packets, name):
	...
	for pkt in packets:
		...
		# DEBUG
		if i % 1000 == 0:
			sys.stdout.write("\r%dB    %f%%" % (total_bytes, 100 * dt / float(MAX_DT)))
			sys.stdout.flush()
		i += 1
	...
	print('Portion of reads signed:       %f' % (0 if total_reads == 0 else total_sig_reads / float(total_reads)))
	print('total writes:                  %f' % total_writes)
	print('Portion of writes signed:      %f' % (0 if total_writes == 0 else total_sig_writes / float(total_writes)))
...
#pdus = gdp_02_pdus + gdp_03_pdus + gdp_04_pdus
pdus = gdp_01_pdus
